...
$(document).ready(function(){
	...
	$("#btnBMDelete").click(function(){
		...
		var $form = $("<form>")
						.attr("action", "/anibuddy/activity/ibookmarkdelete")
						.attr("method", "post")
						.append(
								$("<input>")
									.attr("type", "hidden")
...
<div class="o3-content">
			...
			<c:when test="${not empty bmList }"> <% int bmNo = 1; %>
				<c:forEach items="${bmList }" var="bm">
					<c:forEach items="${list }" var="list">
						<c:if test="${(bm.iNo eq list.iNo) and (bm.userNo eq userno) }">
							<tr>
								<td><input type="checkbox" name="checkRow" value="${bm.bmNo }" /></td>
								<td><a href="/anibuddy/info/view?iNo=${bm.iNo }"><%=bmNo++ %></a></td>
								<td><a href="/anibuddy/info/view?iNo=${bm.iNo }">${list.iName }</a></td>
								<td>${list.iCity1 } ${list.iCity2 }</td>
							</tr>
						</c:if>
					</c:forEach>
				</c:forEach>
	...
	<c:if test="${paging.totalCount ne 0 }">
		<c:import url="./activity_info_bm_paging.jsp" />
	</c:if>
