...
<div class="top">
	<h2>알거리 <span class="h2-inner"> 장소 상세보기</span></h2>

	<div class="top-menu">
		<button type="button" class="btn view-list" onclick="location.href='javascript:history.go(-1)'">목록으로</button>	
		<c:if test="${info.userNo eq userno || adminLogin}">
			<div class="delete-place" id="delete">
				<button type="button" class="btn view-delete" id="btnDelete"
				onclick="if(confirm('정말 삭제하시겠습니까?')==true){location.href='<%=request.getContextPath() %>/info/delete?iNo=${info.iNo }'} else return;">
				삭제하기</button>
			</div>
		</c:if>
	</div>
...
<div class="info-box">
	<div class="place-name">
		<p class="iName">${info.iName }</p>
		<div class="filter-icon">#${info.iFilter }</div>
	</div>

	<div class="info">
		<div class="info-title">주소</div>
		<div class="info-content">${info.iAddress }</div>
	</div>

	<div class="info">
		<div class="info-title">영업시간</div>
		<div class="info-content">${info.iTime }</div>
	</div>

	<div class="info">
		<div class="info-title">연락처</div>
		<div class="info-content">${info.iTel }</div>
	</div>

	<div class="info">
		<div class="info-title">사이트</div>
		<div class="info-content"><a href="http://${info.iDomain }" target="_blank">${info.iDomain }</a></div>
	</div>

	<div class="info">
		<div class="info-title">특이사항</div>
		<div class="info-content">${info.iContent }</div>
	</div>
</div>
...
//주소로 좌표를 검색합니다
geocoder.addressSearch('${info.iAddress}', function(result, status) {
...
// 정상적으로 검색이 완료됐으면 
 if (status === kakao.maps.services.Status.OK) {
    ...
    var infowindow = new kakao.maps.InfoWindow({
        content: '<div class="map-content">${info.iName}</div>'
    });
